  ...
  <div class="app-container">
    ...
    <Form ref="form" :model="model" :messages="messages" :rules="rules" style="padding:20px 20px" @validate="errors=$event">
      ...
      <FormField name="video" label="视频简介:" class="f-full">
        <FileUpload :file="model.videoItem" @input="emitVideoUpload" previewType="shortcut"/>
      </FormField>
...
export default {
  ...
  created() {
    ...
    let product = this.getProductItem();
  ...
  methods: {
    ...
    openSpec(val, oldVal) {
      if (this.model.type == 2 && this.model.type_id != null) {
    ...
    async getSpecs(type_id) {
      this.loading = true;
      let result = await apiGoods.spec(type_id);
      this.model.productItems = [];
    ...
    async getTreeData() {
      let res = await apiCategory.getTree();
